...
- בכדי ולהגביל את ה overfitting אנו נשים מגבלות על על פונקציית החיזוי.
...
- הסכום.
- מרחק העסקה (נניח המיקום של החנות) מהעיסקה האחרונה.
- מרחק העסקה מהכתובת של הלקוח.
- השעה ביום.
- אופי המוצרים שהחנות מוכרת (מכולת, מוצרי חשמל, ביגוד, רכב, נדל"ן, וכו')

ניתן לעשות זאת בערת supervised learning על ידי שימוש במדגם דוגמאות מהעבר.
...
**הערה**: ניתן להחליף את המחרק האוקילידי (נורמת $l_2$) ממדי מרחק אחרים.
...
- החזאי מורכב מאוסף של איזורים קטנים המכונים Vornoi cells המשוייכים לכל דגימה.
...
לדוגמא עם היינו מודדים את המרחק במטרים:
...
במקרה של שיוון בשלב 2:

- נשווה גם את המרחק הממוצע בין ה $\boldsymbol{x}$-ים של כל תווית ונבחר את המרחק הממוצע הקצר ביותר.
...
- ישנם אלגוריתמים רבים לבניה של עץ החלטה. שני האלגוריתמים הנפוצים ביותר הם C4.5 (Ross Quinlan, 1986) ו CART (Breiman *et al.*, 1984).
...
- נתחיל מהשורש ונוסיף nodes נותנים את שגיאת החיזוי הקטנה ביותר על ה train set.
...
- לעלה השמאלי מגיעים 13 דגימות של הונאה ו 108 חוקיות לכן החיזוי יהיה שהעסקה חוקית
- לעלה הימני מגיעים 18 דגימות של הונאה ו 11 חוקיות לכן החיזוי יהיה שהעסקה חשודה כהונאה
...
3. אמפירי נחשב את חוסר ההומוגניות של כל עלה: $Q(\hat{p}_j)$
