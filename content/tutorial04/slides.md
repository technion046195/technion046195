...
* נגדיר כמה מדדים אשר בוחנים עד כמה הפילוג של $\text{y}$ רחוק מלהיות פילוג אשר מייצר דגימות הומוגניות (זאת אומרת פילוג שהוא פונקציית דלתא):
...
*  במקרים שבהם יש במדגם שתי דגימות עם אותו ה $\boldsymbol{x}$ אך $y$ שונה, לא ניתן להגיע למדד חוסר הומוגניות 0.
...
נרצה לבנות עץ החלטה על מנת לחזות את $\text{y}$ על סמך $\mathbf{x}$. נרצה להשתמש במדד חוסר הומגניות חדש מסוג Square Root Gini אשר מוגדר באופן הבא:
$$
...
* בעבור הענף של $\text{x}_1=-1$ הדגימות הרלוונטיות הן:
...
* למרות שלא הגענו לפילוג הומוגני לא נוכל לפצל יותר את הענף כי הערכים של $\text{x}_2$ זהים בעבור שני הדגימות ולכן לא ניתן להבחין בינהם. במקרה זה נבחר את החיזוי באופן שרירותי להיות $1$ ונסיים את הבניה של העץ:
...
- $\text{y}$ : המשתנה האקראי של האם הנוסע שרד או לא.
