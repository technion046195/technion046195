...
בהינתן מדגם מסויים ומדד חוסר הומוגניות מסויים נגדיר את את הציון של עץ החלטה נתון באופן הבא:
...
מכיוון שבכדי למצוא את הפתרון האופטיאמלי יש לעבור על כל העצים האפשריים, אנו נחפש פיתרון שאינו בהכרח האופטימאלי על ידי בניה של העץ בצורה חמדנית (greedy). אנו נתחיל מה root ונוסיף nodes כך שבכל נבחר את ה node אשר מניב את העץ עם מדד החוסר הומוגניות הנמוך ביותר. אנו נעשה זאת על ידי מעבר על כל האופציות האפשריות לבחור את ה node. ממשיכים לפצל את העלים של העץ כל עוד מדד החוסר הומוגניות יורד.

מלבד מקרים שבהם יש במדגם שתי דגימות עם אותו ה $\boldsymbol{x}$ אך $y$ שונה, לא יהיה ניתן להגיע למדד חוסר הומוגניות 0. זאת אומרת שבכל עלה יש דגימות עם תוויות מסוג אחד בלבד. באותם מקרים שבהם לא ניתן להגיע לתווית בודדת לכל עלה, נבחר את תצואת החיוזי להיות הערך הכי שכיח באותו עלה.
...
מתחילים מכל אחד מהעלים ובודקים לכל node, האם הסרה שלו משפרת או לא משנה את הביצועי העץ על ה validation set. במידה וזה אכן המצב מסירים אותו. ממשיכים כך עד שאין עוד מה להסיר.
...
**2)** cבעבור המדגם הנתון, מה קורה במקרה שבו $K=8$? האם שגיאה זו היא שגיאת overfitting או underfitting?
...
נבדוק את בשלות האבטיח שהסטודנט מחזיק בידו על סמך המדגם כולו עם $K=3$. שלושת הנקודות הקרובות ביותר לנקודה $(8,3)$ הינם הנקודות $0$, $3$ ו $4$. מכיוון ששתיים מהם עם תווית של $-1$ אנו נחזה שאבטיח זה הוא בוסר..
...
ה node האופטימאלי כאן הוא הפיצול לפי $\text{x}_2$ ונשים לב שהענף של $\text{x}_1=1$ כבר הומוגני:
...
בעבור העלים אשר הפילוג של התגיות בהם הינו הומוגני החיזוי יהיה מושלם. שגיאות חיזוי יתקבלו רק בעלה של $\text{x}_3=\text{2}=-1$ אשר לא הצליח להגיע לפילוג הומוגני. מכיוון שבחרנו (באופן שרירותי) שהחיזוי בעלה זה יהיה 1 הדגימה יחידה אשר תסווג לא נכון היא דגימה 4. מכאן שהחזאי שבנינו יעשה על המדגם שגיאה אחת מתוך 5, זאת אומרת misclassification rate של $1/5=0.2$.

כפי שציינו קודם, מכיוון שלדגימות 3 ו 4 יש את אותו $\boldsymbol{x}$ אך $y$ שונה לא ניתן להפריד בניהם ותמיד על אחד מהם החיזוי יהיה לא נכון. לכן הציון של $0.2$ הוא הציון המינמאלי שאותו ניתן לקבל על המדגם הזה.
...
מתוך הגרפיים ניתן לראות כי אכן ישנם מאפיינים שיוכלו לסייע לשנו לשפר את החיזוי שלנו. לדוגמא, ניתן לראות כי לנשים היה סיכוי גבוהה בהרבה לשרוד לגברים וכך גם לנוסעים במחלקה הראשונה.
...
ולכן נבחר למיין לפי המגדר. נשמיך באופן זהה לכל שאר ה nodes.
...
לאחר חישוב העץ המלא נשתמש ב validation set על מנת להסיר את הענפים שלא משפרים (או פוגעים) בציון על ה validation set. בדיקה זו מראה שיש ארבעה node של תורמים לשיפור התוצאה ולכן נסיר אותם ונקבל את העץ הסופי הבא:
