...
- **MSA**: החציון של הפילוג המותנה:
    ...
    (כאשר $F_{\text{x}|\mathbf{x}}$ היא פונקציית הפילוג המצרפי של $\text{y}$ בהינתן $\mathbf{x}$.
...
### שיעורך של פונקציות פילוג שיטות א-פרמטריות
...
המדידה האמפירית, $\hat{p}_A$, הינה שיערוך של הההסתברות, $Pr\left(A\right)$, להתרחשות המאורע $A$:
...
1. מחלקים את תחום הערכים ש $\mathbf{x}$ יכול לקבל לתאים (**bins**).
2. לכל תא משערכים את ההסתברות של המאורע ש $\mathbf{x}$ יהיה בתוך התא.
...
**הערה**: פונקציות הגרעיון יהיו תמיד פונקציות פילוג חוקיות. זאת אומרת חיוביות שהאינטרגל עליהם שווה של 1.

##### הוספת פרמטר רוחב

מקובל להוסיף לפונקציות הגרעין פרמטר $h$ אשר שולט ברוחב שלהן באופן הבא:

$$
\phi_h(\boldsymbol{x})=\frac{1}{h^D}\phi\left(\frac{\boldsymbol{x}}{h}\right)
$$

כאשר $D$ הוא המימד של $\mathbf{x}$. החלוקה ב$h^D$ היא על מנת לשמור על הנרמול של הפונקציה.
...
מכיוון שבעבור $i\neq j$ אז $\text{x}^{(i)}$ הוא בלתי תלוי ב $\text{x}^{(j)}$ נוכל במקרים אלו לפרק את התוחלת של המכפלה למכפלת התוחלות. נפריד אם כן את הסכום לסכום על כל המקרים בהם $i=j$ ($N$ מקרים) מהמקרים שבהם $i\neq j$ ($N^2-N$ מקרים):
...
**3)** חזרו על שתי הסעיפים עם הסטוגרמה שמחלקת את התחום ל15 תאים.

**4)** חזרו על שתי הסעיפים הראשונים עם KDE עם פונקציית גרעין של מסוג חלון מרובע ופרמטר רוחב $h=5$
...
\frac{1}{3(5-0)}={2}{15}&0\leq x<5\\
\frac{2}{3(10-5)}={1}{15}&5\leq x<10\\
...
| $y=0$ | $\frac{2}{15}\frac{3}{7}=\frac{2}{35}$  | $\frac{1}{15}\frac{3}{7}=\frac{1}{35}$   | $0\frac{3}{7}=0$                       |
...
| $y=1$ | $0$         | $0$           | $0$         | $0$         | $\frac{2}{7}$ | $0$         | $\frac{1}{7}$ | $0$           | $0$         | $0$           | $0$           | $0$           | $0$           | $0$           | $0$               |
...
על מנת לראות את התלות של ה ECDF בגודל המדגם נחזור על החישוב עם כמויות קטנות יותר של דגמים במדגם. אנו נבחר בארקאי $N=10,10,1000$ דגמים מהמדגם ונחזור על החישוב. התוצאה:
...
- כאשר **מספר התאים גדול**, כל תא יהיה צר ומקור השגיאה העיקרי ינבע מה**אקראיות** של המדגם הגורמת לשינויים גדולים במספר היחסי של נקודות אשר נופלות בכל תא. שגיאה זו נובעת מה variance של המשערך. שגיאה זו תלך ותקטן ככל שנגדיל את כמות הדגמים במדגם.
...
$$
\hat{y}=\underset{y}{\arg\max}\ \hat{p}_{\text{y},\mathcal{D}}(y)=0
$$

הסיבה שזהו החיזוי האידאלי נובעת ישירות מן העובדה שיש במדגם יותר נסיעות שהתרחשו מחוץ לשעות העבודה, לכן שיערכנו שיש סיכוי מעט יותר גדול שנסיעה אקראית תתרחש בשעות אלה ולכן יש לנו סיכוי קטן יותר לטעות בעבור חיזוי זה.
