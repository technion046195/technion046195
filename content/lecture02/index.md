...
> כאשר אנחנו מתלבטים האם לקחת איתנו מטריה ביציאה מהבית, אנו למעשה מנסים לבצע חיזוי של האם ירד גשם או לא על סמך פרטי מידע שיש בידינו, כגון התחזית שמענו, כמות העננים בשמים, צבע העננים וכו'.
...
בבעיות חיזוי קלאסיות, אנו מניחים שהפילוג של כל המשתנים האקראיים ידוע, וכי האתגר הוא מציאת החזאי האופטימאלי על סמך הפילוג. לעומת זאת, בבעיות supervised learing (ובבעיות במערכות לומדות באופן כללי) אנו מניחים כי הפילוג אינו ידוע ובמקומו נתון לנו מדגם של דגימות מתוך אותו פילוג. את החזאי נאלץ כעת לבנות על סמך המדגם במקום על סמך הפילוג. במהלך הקורס אנו נדון נכיר שיטות שונות לבנות חזאים באופן זה ונדון בבעיות הקיימות בשיטות אלו.
...
מיכוון שבעבור כל חיזוי שנבחר אנו מצפים לשגיאה כל שהיא, אנו צריכים להגדיר דרך להשוות בין הטעויות שאותם מבצעים החזאים שונים. אנו צריכים להחליט לדוגמא איך נבחר בין חזאי שעושה כל הזמן שגיאות בינוניות לבין חזאי אשר רוב הזמן עושה שגיאות ממש קטנות אך פעם בכמה זמן עושה שגיאה מאד גדולה. הדרך הנפוצה לעשות זאת היא על ידי שימוש בפונקציית מחיר (cost function).

#### Cost function (פנקציית המחיר)

פונקציית המחיר היא פונקציה $C(h)$ אשר מעניקה לכל חזאי ציון. לרוב ככל שהציון נמוך יותר החזאי טוב יותר. אנו נגדיר את החזאי האופטימאלי כחזאי בעל הציון הנמוך ביותר מבין כל החזאים האפשריים. נסמן את החזאי האופטימאלי ב $h^*$:
...
ננסה להגדיר מחדש את הבעיה של חיזוי זמן הנסיעה בעזרת הכלים הנ"ל.
...
- $\mathcal{D}$ - המדגם הנתון של הזוגות של (מספר מכוניות, זמן נסיעה).
- בעיה זו הינה בעיית רגרסיה.
- בהרצאה זו נבחר להשתמש ב MSE כפונקציית ה risk\cost שלנו.
...
באופן כללי ניתן לחלק את השיטות לפתרון בעיות spuervised learning לשני גישות:
...
**גישה דיסקרימינטיבית (discriminative)** - בשיטה זו ננסה לבנות חזאי אשר מייצר חיזויים טובים על הדגימות מהמדגם בתקווה שהוא יתן חיזויים טובים גם לדגימות מחוץ למדגם.

לכל אחד מהגישות יש את היתרונות והחסרונות שלה. במהלך הקורס נכיר אלגוריתמים משני הגישות ונעמוד על ההבדלים בניהם.

## Empirical risk minimization (ERM)

השיטה הגנרטיבית הפופולרית ביותר היא ERM והיא מבוססת על השימוש בתוחלת אמפירית בכדי לקרב את התוחלת הלא ידועה. התוחלת האמפירית מחליפה את הסכימה / אנטרגל על פני הפילוג בממוצע על פני סט של דגימות מתוך הפילוג. אנו נשתמש בסימון בסימון $\hat{\mathbb{E}}_{\mathcal{D}}$ על מנת לסמן את התחולת האמפירית המבוססת על המדגם $\mathcal{D}$.
...
במקרה שלנו, אנו נחליף את התוחלת על פונקציית ה loss במיצוע של הערכים שהיא מפיקה על הנקודות מהמדגם על מנת לקבל את הrisk האמפירי:
...
בבעיית ה ERM אנו ננסה למזער את ה risk האמפירי (במקום את ה risk האמיתי)
...
ישנם שתי סיבות עיקריות שבגללם נרצה לעבוד עם מודלים פרמטריים:

1. כפי שציינו קודם, הם מגבילים את מרחב החיפוש של המודל ובכך מסיעים למזער את ה overfitting. נרחיב על כך בהרצאה הבאה.
2. נוח לעבוד איתם. אנו למעשה ממפים כלפונקציה מהמשפחה הפרמטרית לוקטור. היתרון בעבודה עם וקטורים הינו שיש לנו  עשיר של כלים בהם אנו יכולים להשתמש. לדוגמא, מכיוון שניתן לגזור לפי וקטורים, נוכל להשתמש ב gradient decent על מנת לחפש את המודל האופטימאלי במרחב המודלים.

מיכוון שכל וקטור כעת מגדיר מודל מסויים (ולהיפך) ניתן לרשום את בעיית האופטימיזציה של מציאת המודל האופטימאלי כבעיית אופטימיזציה על וקטור הפרמטרים (במקום על $h$):
...
לשם הנוחות בכדי לשמור על הכתיב הוקטורי של המודל נפריד לרוב את איבר ה bias משאר הפרמטרים. לרובנסמן אותו בעזרת $b$ או $\theta_0$:
...
נשתמש במודל לינראי ובפתרון של בעיית ה LLS בכדי למצוא חזאי לבעיית שיערוך זמן הנסיעה. נתחיל במודל ללא היסט:
...
את קלט החדש $\boldsymbol{x}_{\text{new}}$ מקובל לכנות וקטור ה**מאפיינים (features)**. השימוש במאפיינים מאפשר להו מספרים דברים:
...
נתסכל כעת על מקרה בו אין פתרון סגור לפרמטריים האופטימאלים (כפי שיש במקרה של LLS).

נניח ואנו רוצים לנסות להשתמש במודל מהצורה:
...
נרצה למצוא את הפרמטרים האופטימאליים של מודל זה בעזרת ERM תחת פונקציית המחיר של RMSE. זאת אומרת שבעיית האופטימיזציה שלנו תהיה:
...
כאשר ניתן להפתר מהשורש ומה $\frac{1}{N}$ מבלי לשנות את בעיית האופטימיזציה:
...
מכיוון שבבעיה זו לא ניתן להגיע לפתרון סגור על ידי גזירה של פונקציית המטרה והשוואה ל-0 נאלץ להשתמש בשיטות אחרות. אחת השיטות הנפוצות במערכות לומדות (בעיקר בעבודה עם רשתות נוירונים, אך לא רק) לפתרון בעיות אופטימיזציה מסוג זה הינה להשתמש ב gradient descent אותו הצגנו בקצרה בתרגול 1.

הרעיון מאחרי Gradient descent הוא פשוט. האלגוריתם מנסה למצוא מינימום לוקאלי של פונקציית מטרה כל שהיא על ידי ניסיונות להתקדם בכיוון שבו הפונקציה יורדת הכי מהר.
...
לאלגוריתמים אטרטיביים מסוג זה, אשר מנסים בכל איטרציה לשפר את מצב לעומת המצב הנוכחי (מבלי התייחס צורה הגלובאלית של הפונקציה) אנו קוראים אלגוריתמים חמדנים (greedy). כפי שציינו קודם אלגוריתמים כאלה לא מתיימרים להתכנס לאפטימום הגלובאלי אלא רק ינסו להשתפר עד אשר יגיעו לאופטימום לוקאלי.

הדרישה היחידה לשם השימוש באלגוריתם הינה היכולת לחשב את הנגזרת של פונקציית המטרה. בהינתן הנגזרת שלבי האלגוריתם הם כדלקמן:

- מאתחלים את הפרמטרים שלפיהם רוצים למצוא את המינימום, במקרה שלנו $\boldsymbol{\theta}$, בנקודה לערך התחלתי כל שהוא: $\boldsymbol{\theta}^{(0)}$
...
### הבעיה העיקרית של gradient descent
