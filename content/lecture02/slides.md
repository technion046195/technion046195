...
<br/>
<br/>
$\{\mathbb{x}^{(i)}, \text{y}^{(i)}\}$ הינו בלתי תלוי סטטיסטית בזוג המשתנים $\{\mathbb{x}^{(j)}, \text{y}^{(j)}\}$ כאשר $i\neq j$.
...
- הסיכון מוגדרת על ידי תוחלת על פני הפילוג של המשתנים האקראיים בבעיה שהוא כאמור לא ידוע.
...
- נוכל במקום לנסות ולחשב את המחיר באופן אנליטי לשערך אותו על סמך אוסף של דוגמאות (מדגם).
...
- מתייחס למקרים בהם פונקציית המחיר מוגדרת כפונקציית סיכון.
