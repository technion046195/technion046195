...
- **Observations \ measurements** (תצפיות או מדידיות): $\mathbf{x}$ - הוקטור האקראי אשר שלפיו נרצה לבצע את החיזוי. (לרוב וקטור)
...
<br/>
<br/>
זאת אומרת שבעבור זוג אינדקסים $i\neq j$ הדגימה $\{\mathbb{x}^{(i)}, \text{y}^{(i)}\}$ הינה בלתי תלויה סטטיסטית בדגימה $\{\mathbb{x}^{(j)}, \text{y}^{(j)}\}$.
