...
**מטרת תהליך הלימוד** היא, אם כן, לבחור את הפונקציה האופטימלית כתלות במדגם $\mathcal{D}$, $h^{*}_{\mathcal{D}}$ מתוך מחלקת ההשעערות $H$, שמביאה את פונקציית הסיכון למינימום.  
...
- עבור בעיות סיווג נקבל:
 $$
 \hat{L}_{\mathcal{D}}(h) = \frac{1}{N}\sum_{i=1}^{N}I(h(\boldsymbol{x}^{(i)})\neq h_0(\boldsymbol{x}^{(i)}))
 $$
