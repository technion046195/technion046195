...
עבור השערה $h^{*}_{\mathcal{D}}$ הממזערת את הסיכון האמפירי מקיימת לכל $\varepsilon > 0$ המקיימת 

 $$
 P(R(h^{*}_{\mathcal{D}})>\varepsilon) < |H|e^{-\varepsilon n}
 $$

התוחלת שלה, $E(R(h^{*}_{\mathcal{D}}))$, חסומה על ידי 

 $$
 E[R(h^{*}_{\mathcal{D}})] < \frac{1+\log(|H|)}{N}
 $$
...
### הוכחת משפט 1 - ניסוח חסם על התחולת 

נחשב את התוחלת עבור השערה $h^{*}_{\mathcal{D}}$ הממזערת את הסיכון האמפירי מקיימת לכל $\varepsilon > 0$ המקיימת 
...
$$
\begin{aligned}
E[R(h^{*}_{\mathcal{D}})] &= \int_{0}^{\infty} P(R(h^{*}_{\mathcal{D}}) \ge x)dx \\
&= \int_{0}^{u} P(R(h^{*}_{\mathcal{D}}) \ge x)dx + \int_{u}^{\infty} P(R(h^{*}_{\mathcal{D}}) \ge x)dx
&\le u + \int_{u}^{\infty} P(R(h^{*}_{\mathcal{D}}) \ge x)dx
