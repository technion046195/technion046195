...
**מטרת תהליך הלימוד** היא, אם כן, לבחור פונקציה $\hat{h}_{\mathcal{D}}\in H$ (כתלות במדגם $\mathcal{D}$) אשר מביאה את מדד הביצועים $L(h)$ למינימום.

הבעיה היא כמובן ש-$L(h)$ אינו ניתן לחישוב מתוך מדגם סופי!

### הערות

- חשוב להדגיש כי הדוגמאות $\{x^{(i)}\}$ נבחרות לפי אותו פילוג $P_X$ המשמש בהגדרת מדד הביצועים. דבר זה יאפשר קבלת חסמים על קצב ושגיאת הלימוד שאינם תלויים ב-$P_X$.
...
כלומר, בהינתן המדגם $\mathcal{D}=\{\boldsymbol{x}^{(i)}, y^{(i)}\}_{i=1}^N$ , נבחר את ההשערה $\hat{h}_{\mathcal{D}}$ באופן הבא:
...
נניח מעתה כי  $\hat{h}_{\mathcal{D}}$ היא אכן הפונקציה הנבחרת על ידי אלגוריתם הלמידה שלנו.  בפרט, אנו מניחים כי ניתן למצוא את המינימום הגלובאלי של $\hat{L}_{\mathcal{D}}(h)$ , מבלי להתייחס לקושי החישובי הכרוך בכך.
...
נסמן - $h^* \in \underset{h\in H}{\arg \min} \hat{L}(h)$ - ההשערה האופטימלית שאינה ניתנת לחישוב. 
...
## חסמים עבור מחלקת השערות סופית
 ...
 מטרתנו למצוא חסמים על קריטריון הביצועים $L(\hat{h}_{\mathcal{D}})$, כאשר $\hat{h}_{\mathcal{D}}$ היא הפונקציה (ההשערה) המביאה למינימום את המחיר האמפירי $\hat{L}_{\mathcal{D}}(h)$.

 נשים לב כי במקרה הבינארי המחיר האמפרי איננו אלא השגיאה האמפירית (למה?). 

 ראשית נעסוק במקרה בו $h_0\in H$, כלומר במקרה בו פונקציה המטרה $h_0$ כלולה בתוך קבוצת ההשערות $H$, כלומר $L^*=\underset{h \in H}{\min}L(h)=0$. 
...
החסם שקיבלנו מאפשר לנו לבחור את גודל המדגם $N$ המבטיח שגיאה קטנה כרצוננו, ובהתסברות גבוהה כרצוננו, אם $N>\frac{1}{\varepsilon} \log\frac{|H|}{\delta}$, נקבל כי $L(\hat{h}_{\mathcal{D}})$ בהסתברות $1-\delta$ לפחות. 


#### מספר מונחים בסיסיים בלמידה חישובית:

 אלגוריתם כלשהו לבחירת $\hat{h}_{\mathcal{D}} \in H$  שעבורו  $P(L(\hat{h}_{\mathcal{D}})>\varepsilon) \rightarrow 0$ כאשר  $N \rightarrow \infty$ (לכל $ h_0 \in H $) נקרא אלגוריתם Probably Approximately Correct או בקיצור PAC. קבוצת השערות $H$ שעבורה קיים אלגוריתם PAC נקראת ברת-למידה (Learnable). 
...
מטרתנו בהוכחה זאת היא לחסום את $P(L(\hat{h}_{\mathcal{D}})-L^* > \varepsilon)$. 
...
L(\hat{h}_{\mathcal{D}}) - \hat{L}_{\mathcal{D}}(\hat{h}_{\mathcal{D}}) + \hat{L}_{\mathcal{D}}(\hat{h}_{\mathcal{D}}) - L^* \\
&\le [L(\hat{h}_{\mathcal{D}}) - \hat{L}_{\mathcal{D}}(\hat{h}_{\mathcal{D}})] + [\hat{L}_{\mathcal{D}}(h^*) - L^*]  \\
...
נשים לב שמאורע המקסימום שקול למאורע איחוד המאורעות ולכן נוכל להשתמש בחסם האיחוד במעבד מהשורה הראשונה לשנייה. 
