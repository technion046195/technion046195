...
- **מודל פרמטרי** - אוסף $H$ של פונקציות $H: \mathcal{X} \rightarrow \mathcal{Y}$, שמתוכו נבחר את הפונקציה $\hat{h}$ אשר משערכת את פונקציית המטרה $h$. $H$ תכונה כאן **מחלקת ההשערות**.
...
**מטרת תהליך הלימוד** היא, אם כן, לבחור פונקציה $h^{*}_{\mathcal{D}} \in H$ (כתלות במדגם $\mathcal{D}$) אשר מביאה את מדד הביצועים $L(h)$ למינימום.
...
$$
\h^{*}_{\mathcal{D}}\in \underset{{h\in H}}{\arg\min} \hat{L}_{\mathcal{D}}(h), \qquad \hat{L}_{\mathcal{D}}(h) = \frac{1}{N}\sum_{i=1}^{N}\mathcal{l}(h(x^{(i)}),h_0(x^{(i)})) 
$$
...
נניח מעתה כי  $h^{*}_{\mathcal{D}}$ היא אכן הפונקציה הנבחרת על ידי אלגוריתם הלמידה שלנו.  בפרט, אנו מניחים כי ניתן למצוא את המינימום הגלובאלי של $\hat{L}_{\mathcal{D}}(h)$ , מבלי להתייחס לקושי החישובי הכרוך בכך.
...
![](./assets/pac-1.png)
 ...
 $$
 L(h^{*}_{\mathcal{D}})=L(h^*)+[L(h^{*}_{\mathcal{D}})-L(h^*)]
 $$
...
-	האיבר השני הוא שגיאת השערוך (בדומה למשתנה השונות), ומבטא את השגיאה הנובעת מסופיות המדגם עקב כך שהפונקציה הנבחרת  $h^{*}_{\mathcal{D}}$ אינה האופטימלית (מתוך  $H$). זאת מכיוון שאנו מבצעים מינימיזציה של המחיר האמפירי   במקום של קריטריון הביצועים. 
...
## חסמים עבור מחלקת השערות סופית
 ...
 מטרתנו למצוא חסמים על קריטריון הביצועים $L(h^{*}_{\mathcal{D}})$, כאשר $h^{*}_{\mathcal{D}}$ היא הפונקציה (ההשערה) המביאה למינימום את המחיר האמפירי $\hat{L}_{\mathcal{D}}(h)$.
 ...
 ### משפט 1
 נניח כי $|H| < \infty$ וכן $h_0 \in H$, כלומר $L^*=0$. אזי, השערה $h^{*}_{\mathcal{D}}$ הממזערת את השגיאה האמפירית מקיימת לכל $\varepsilon > 0$

 $$
 P(L(h^{*}_{\mathcal{D}})>\varepsilon) < |H|e^{-\varepsilon n}
 $$
...
- לכל $\delta>0$ מתקיים בהתסברות של $(1-\delta)$ לפחות $L(h^{*}_{\mathcal{D}}) < \frac{1}{N} \log\frac{|H|}{\delta}$
