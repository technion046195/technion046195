...
1. **Curse of dimensionality**: השיטות הלא פרמטריות לומדות את הפילוג בכל איזור על פי הדגימות שנמצאות באותו איזור, באופן בלתי תלוי באיזורים האחרים במרחב. עקב כך, שיטות אלו דורשות כמות דגימות כזו שתכסה בצורה מספיק טובה את כל האיזורים הסבירים של מרחב הדגימות האפשריות. הבעיה היא שהגדול האפקטיבי של מרחב הדגימות גדל בצורה מעריכית עם המימד של הדגימות (האורך של הוקטור $\mathbf{x}$), לכן בעובר מקרים שבהם המימד של דגימות הוא גדול, כמות הדגימות לרוב לא יחסכו בצורה טובה את המרחב ולכן השיערוך יהיה מאד לא מדוייק.
2. המודלים המתקבלים בשיערוך לא פרמטרי לרוב אינם פונקציה שנוח לעבוד איתן. לגודמא, על מנת לחשב את הצפיפות בנוקדה מסויימת בעזרת KDE יש לבצע סכמה על כל הנקוןדות שנמצאות ב train set.

### הגישה הפרמטרית

שיטה דומה עושה שימוש במודלים פרמטרים בדומה לאופן שבו הדבר נעשה בגישה הדיסקרימינטיבית. בשיטה זו אנו נגביל את החיפוש של הפונקציה המתארת את הפלוג למשפחה פרמטרית מסויימת, ונחפש את הפרמטרים האופטימאלייים של המודל. לרוב הפונקציה שאותה ננסה למדל הינה פונקציית צפיפות הפילוג (הPDF). חושב לשים לב שבניגוד לשימוש במודלים פרמטרים בגישה הדטרמיניסטית שם לא הייתה שום מגבלה על המודל הפרמטרי, כאן המודל חייב לייצר פילוג חוקי בעבור כל בחירה של פרמטרים (במקרה של PDF זה אומר פונקציה חיובית שאינטרגל עליה נותן 1)

ישנם שני דרכים להתייחס לפרמטרים של מודל. שני דרכים אלו מגיעים משני גישות הקיימות בתחחום של תורת השיערוך וכל דרך מובילה לדרך מעט שונה לבחירה של הפרמטרים האופטימאליים. בשני השיטות אנו נסמן את וקטור הםרמטריים של המודל ב $\boldsymbol{\theta}$.

#### הגישה הלא-בייסיאנית (המכונה גם: קלאסית או תדירותית (**Frequintist**))

בגישה זו נתייחס לפרמטרים באופן דומה לאופן שבו התייחסנו עליהם כאשר עסקנו בשיטות בגנרטיביות. תחת גישה זו אין כל העדפה של ערך מסויים של הפרמטריים על פני ערך אחר. את המודל הפרמטרי להסתברות / צפיפות הסתברות של משתנה אקראי $\mathbf{x}$ נסמן ב:
...
הדרך הנפוצה ביותר לבחור את הערך של $\boldsymbol{\theta}$ תחת הגישה הלא באייסאנית היא בעזרת MLE. בשיטה זו נחפש את הערך של $\boldsymbol{\theta}$ אשר מסביר בצורה הכי טובה את המדגם הנתוון. נסמן ב $p_{\mathcal{D}}(\mathcal{D};\boldsymbol{\theta})$ גם את ההסתברות לקבלת מדגם כל שהוא $\mathcal{D}=\{\boldsymbol{x}^{(i)}\}$. גודל זה מכונה ה**סבירות** (**likelihood**) של המדגם כפונקציה של $\boldsymbol{\theta}$. על מנת להדגיש את העובדה שהמדגם הוא למעשה גודל ידוע ואילו המשתנה שאותו נרצה לבדוק הינו $\boldsymbol{\theta}$ מקובל לסמן את פונקציית ה likelihood באפן הבא:

$$
\mathcal{L}(\boldsymbol{\theta};\mathcal{D})\triangleq p_{\mathcal{D}}(D;\boldsymbol{\theta})
$$

משערך ה MLE הוא הפילוג עם הפרמטרים אשר ממקסמים את פונקציית ה likelihood (או ממזער את המינוס שלה):
...
במקרים רבים נוכל להחליף את המכפלה על כל הדגימות בסכום, על ידי מקסימום של הלוג של פוהקציית ה likelihood (אשר בזכות המונוטוניות העולה של פונקציית ה log יניב את אותם פרמטריים אופטימאליים):
...
הדרך הנפוצה ביותר לבחור את הערך של $\boldsymbol{\theta}$ תחת הגישה הבאייסאנית היא בעזרת MAP. בשיטה זו נחפש את הערך הכי סביר של $\boldsymbol{\theta}$ בהינתן המדגם. הפילוג של $\boldsymbol{\theta}$ ביהנתן המדגם, $p_{\boldsymbol{\theta}|\mathcal{D}}(\boldsymbol{\theta}|\mathcal{D})$, מכונה ה**פילוג הפוסטריורי** (**posterior distribution**) או **א-פוסטריורי** (**a posteriori distribution**) (או הפילוג בדיעבד), זאת אומרת, הפילוג אחרי שראינו את המדגם. על פי בייס

משערך ה MAP הוא וקטור הפרמטרים אשר ממקסמים את ההסתרות ה א-פוסטריורי:
...
$$
 ...
 כאשר הדגימות במדגם **בהינתן** $\boldsymbol{\theta}$ הם i.i.d ההסתברות המותנית תהיה:
...
גם כאן נוכל להפוך את המכפלה לסכום על ידיד מיזעור של הלוג של הפונקציה:
...
התנאי $\theta\geq x^{(i)}$ לכל $i$ שקול ל $\theta>\max_i\{x^{(i)}\}$. מצד אחד נרצה לקיים תנאי זה בכדי שה likelihood לא יתאפס, מצד שאני נרצה ש $\theta$ יהיה כמה שיותר קטן בכדי למקסם את $1/\theta^N$. לכן נבחר את ה $\theta$ מינימאלי אשר מקיים את התנאי:
