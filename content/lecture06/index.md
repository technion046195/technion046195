...
לפני שנציג את אלגוריתם ה SVM נתעכב לרגע על המשוואת המישור $\boldsymbol{w}^{\top}\boldsymbol{x}+b=0$ כדי לקבל קצת אינטואיציה לגבי התפקיד של $\boldsymbol{w}$ ו $b$ במשוואה זו.
...
כדי לחשב את $\boldsymbol{w}$ באופן מפורש יש לחשב את $\Phi$, אך ניתן להמנע מכך עם מציבים את הנוסחא ל $\boldsymbol{w}$ ישירות לתוך המסווג:

כדי להמנע מהחישוב של $\Phi$ גם במסווג נשתמש בעובדה ש:
