...
$$
\begin{aligned}
p_{\text{y}}(0)\prod_{d=1}^D p_{\text{x}_d|\text{y}}(x_d|1)\overset{?}{>}
&p_{\text{y}}(1)\prod_{d=1}^D p_{\text{x}_d|\text{y}}(x_d|0)\\
0.5 \times 1 \times 1 \times 0.75 \times 0.5 \times 0.5 \overset{?}{>}
&0.5 \times 0.5 \times 0 \times 0.25 \times 0 \times 0.5\\
0.09375 \overset{?}{>}
& 0
\end{aligned}
$$
...
נסמן ב $p_{\mathcal{D}}(\mathcal{D};\boldsymbol{\theta})$ את ההסתברות לקבלת המדגם הנתון $\mathcal{D}=\{\boldsymbol{x}^{(i)}\}$ על פי המודל שבידינו. גודל זה מכונה ה**סבירות** (**likelihood**) של המדגם $\boldsymbol{\theta}$. אנו מעוניינים למצוא את הפרמטרים $\boldsymbol{\theta}$ אשר מניבים את הסבירות הכי גבוהה. על מנת להדגיש את העובדה שהמדגם הוא למעשה גודל ידוע ואילו הגודל הלא ידוע, שאותו נרצה לבדוק, הינו $\boldsymbol{\theta}$, מקובל לסמן את פונקציית ה likelihood באופן הבא:
...
זו למעשה ממוצע ממושכל בין הערך הממוצע של $x$ במדגם לבין $\mu_{\mu}$. בעבור הדגומא שלנו נקבל::
...
בשיטה זו אנו נשתמש במודל של פילוג נורמאלי וב MLE על מנת לשערך את הפילוג המותנה של $\mathbf{x}$ בהינתן $\text{y}$. אנו למעשה צריכים לשערך מודל נורמלי אחד בעבור כל אחת מ $C$ המחלקות ש $\text{y}$ יכול לקבל. זאת אומרת שאנו נרצה לשערך את הפרמטרים הבאים:
...
ניתן לפתור את הבעיה הזו על ידי גזירה והשוואה ל-0. הפיתוח בעבור $\Sigma_c$ הוא מעט מורכב ואנו לא נראה אותו בקורס זה ונקפות ישר לפתרון. נראה אבל את החישוב של $\boldsymbol{\mu}_c$
