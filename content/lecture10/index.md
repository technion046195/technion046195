...
המרחק בין מישור ההפרדה לשפה של ה margin שווה ל $\frac{1}{\lVert\boldsymbol{w}\rVert}$ והרוחב הכולל של ה margin הינו $\frac{2}{\lVert\boldsymbol{w}\rVert}$. בעיית האופטימיזציה, שמנסה למזער את $\lVert\boldsymbol{w}\rVert$, מחפשת את הפרמטרים של המישור בעל ה margin הגדול ביותר אשר מקיים תנאי זה.
